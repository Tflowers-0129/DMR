...
class TBN(nn.Module):
    ...
    def _prepare_tbn(self):#delete the 'fc' layer, in order to make the multimodal fusion
    ...
    def forward(self, input):
        ...
        for m in self.modality:
            ...
            base_model = getattr(self, m.lower()) #base_model = BNInception
           
            if (m == 'RGB' or m == 'Flow'):
                #BNInception-output before global pooling RGB&FLow(7*7) 
                base_out = base_model(input[m].view((-1, sample_len) + input[m].size()[-2:]))
                concatenated.append(base_out)

            
            elif (m == 'STFT' or m == 'STFT_2'):
                base_out = base_model(input[m].view((-1, sample_len) + input[m].size()[-2:]))#2*2
                
                # 目标输出大小
                output_size = (64, 1024, 7, 7)
                # 执行上采样操作
                base_out = F.interpolate(base_out, size=output_size[2:], mode='nearest')
                concatenated.append(base_out)
            output = concatenated
        return output
