...
def _train(args):
    ...
    for task in range(data_manager.nb_tasks):
        ...
        if task != 0:
            ci = model.Confusion_Index()
            CI.append(ci)
        save_path = os.path.join(f"./{args['model_name']}_confusion_index_w_mix.txt")
        np.savetxt(save_path, CI)
...
def _set_random():
    torch.manual_seed(1)
    torch.cuda.manual_seed(1)  # type: ignore
    torch.cuda.manual_seed_all(1) # type: ignore
