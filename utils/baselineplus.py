    ...
    def forward(self, inputs):
        ...
        if len(self.modality) > 1:
            if len(self.modality) == 4:
                ...
                #res_v = out_vision
                #res_s = out_sensor
                out_v = out_vision * self.sa_1(out_vision)  #type: ignore
                out_s = out_sensor * (self.tsa(out_sensor)['dim1']+self.tsa(out_sensor)['dim2']) #type: ignore
            ...
            base_out = inputs[0]
            base_out = self.avgpool_3(base_out)
            base_out = base_out.squeeze(dim=2)
            base_out = base_out.squeeze(dim=2)
            base_out = {"mire": base_out}
            return base_out
